...
Map<Vector, Aligned> pyvec(const PyArrayRef &ref) {
    auto obj = ref.borrow();
    if (!(PyArray_FLAGS(obj) & NPY_CARRAY))
        throw std::runtime_error("unmappable ndarray");
    if (PyArray_TYPE(obj) != NPY_DOUBLE)
        throw std::runtime_error("ndarray is not of type double");
...
Map<const Vector, Aligned> const_pyvec(const PyArrayRef &ref) {
    auto obj = ref.borrow();
    if (!(PyArray_FLAGS(obj) & NPY_CARRAY_RO))
        throw std::runtime_error("unmappable ndarray");
    if (PyArray_TYPE(obj) != NPY_DOUBLE)
        throw std::runtime_error("ndarray is not of type double");
    if (PyArray_NDIM(obj) != 1) throw std::runtime_error("expected a vector");
...
int const_matrix_converter(PyObject *obj, void *dest_) {
    ...
    *dest = PyArray_FromAny(obj, PyArray_DescrFromType(NPY_DOUBLE), 1, 2,
                            NPY_IN_ARRAY, NULL);
    return 1;
}
