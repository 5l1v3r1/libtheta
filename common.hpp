...
#define ENSURE(x)                                                   \
    do {                                                            \
        if (UNLIKELY(!(x))) throw std::runtime_error("FATAL: " #x); \
    } while (false)
#else
...
public:
    PythonError() { ENSURE(PyErr_Occurred()); }
...
// note: return values of const methods are non-const because Python/C API is
// not const-aware
...
public:
    static PyRef newref(PyObjectT *obj) { return PyRef(obj); }
    ...
    PyRef(decltype(nullptr)) : obj(NULL) {}

    PyRef(const PyRef &other) : obj(other.obj) { Py_XINCREF(obj); }

    PyRef(PyRef &&other) : obj(NULL) { std::swap(obj, other.obj); }
    ...
    PyRef &operator=(PyRef &&other) { std::swap(obj, other.obj); }
    ...
    PyObjectT *borrow() const { return obj; }

    PyObjectT &operator*() const { return *obj; }
    ...
    operator dummy_bool_type() const { return obj ? &PyRef::dummy_true : 0; }

private:
    PyRef(PyObjectT *obj_) : obj(obj_) {}
...
typedef PyObjectRef PyFuncWithKeywords(const PyObjectRef &self,
                                       const PyObjectRef &args,
                                       const PyObjectRef &kwargs);
...
int pyinitproc(PyObject *self, PyObject *args, PyObject *kwargs) {
    try {
        return func(borrow(self), borrow(args), borrow(kwargs)).newref() == NULL
                   ? -1
                   : 0;
    } catch (const PythonError &e) {
        assert(PyErr_Occurred());
...
PyArrayRef pyvec(const DenseBase<Derived> &vec) {
    ...
    PyArrayRef result = newref(PyArray_EMPTY(1, dims, NPY_DOUBLE, 0))
                            .template cast<PyArrayObject>();
...
PyArrayRef pymat(const MatrixBase<Derived> &mat) {
    ...
    PyArrayRef result = newref(PyArray_EMPTY(2, dims, NPY_DOUBLE, 0))
                            .template cast<PyArrayObject>();
    ENSURE(PyArray_Check(result.borrow()));
    pymat(result) = mat;
    return result;
